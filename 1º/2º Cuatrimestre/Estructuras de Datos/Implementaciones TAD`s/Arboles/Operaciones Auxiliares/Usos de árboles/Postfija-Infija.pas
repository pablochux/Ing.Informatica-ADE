...
2. Si se encunetra un operador, se buscan los dos números anteriores que estén en la pila y se construye un arbol con el operador como raiz y como hijos los nodos que estaban en la pila que son los operandos
}

{Operacion auxiliar}
{Determinar si lo que se le pasa es un numero o un operador. 
TRUE -> Numero; 
FALSE -> OPERANDO}
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			{Operando 1}
			{Se van haciendo llamadas recursivas por todos los nodos del árbol}
			r1:= Evaluar(hi);
			{Operando 2}
			r2:= Evaluar(hd);
